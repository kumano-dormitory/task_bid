...
def bid_lack(user: User, db: Session):
    ...
    for bid in bids:
        ...
        task = slot.task
        exp_assignees = [
            exp_assignee for exp_assignee in assignees if task in exp_assignee.exp_task
        ]
...
def bid_close(bid_id: str, db: Session):
    ...
    bidders = (
        db.execute(
            select(Bidder).filter(Bidder.bid_id == bid_id).order_by(Bidder.point)
        )
        .scalars()
        .all()
    )
    ...
    if exp_bidders_len < task.exp_woker_num:
        ...
        if not_exp_bidders_len == 0:
            ...
            for index in range(not_exp_bidders_len):
                slot.assignees.append(inexp_bidders[index].user)
            for index in range(
                task.exp_woker_num, task.exp_woker_num + blank_len - not_exp_bidders_len
            ):
                slot.assignees.append(exp_bidders[index].user)
            db.commit()
            return slot_response(slot)
...
def close_all_bid(db: Session):
    bids = (
        db.execute(select(Bid).filter(Bid.close_time < datetime.datetime.now()))
        .scalars()
        .all()
    )
    closed_bid_id_list = []
