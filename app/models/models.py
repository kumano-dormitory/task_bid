...
class Bid(Base):
    ...
    slot_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("slot.id",ondelete="CASCADE"))
...
class Bidder(Base):
    ...
    bid_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("bid.id",ondelete="CASCADE"),primary_key=True)
...
class Slot(Base):
    ...
    creater_id:Mapped[Optional[uuid.UUID]]=mapped_column(ForeignKey("user.id",ondelete="SET NULL"))
